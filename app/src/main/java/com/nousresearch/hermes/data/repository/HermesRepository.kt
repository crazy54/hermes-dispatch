...
class HermesRepository @Inject constructor(
    ...
    suspend fun listProfiles(config: ConnectionConfig): Result<List<String>> = runCatching {
        // Profiles come from /v1/capabilities or fall back to ["default"]
        val response = apiService.getCapabilities(url(config, "../v1/capabilities"), auth(config))
        if (!response.isSuccessful) return@runCatching listOf("default")
        response.body()?.profiles?.takeIf { it.isNotEmpty() } ?: listOf("default")
    }
    ...
    /** Channels are not yet exposed by the gateway — returns empty list gracefully. */
